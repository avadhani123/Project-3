...
def get_user_tweets(user):
	ident= "twitter_{}".format(user) 
	if ident in CACHE_DICTION: #checks if data is within the cached diciton
		print(user)
		pass #if not printed
	else:
		print(user) #Retrieves internet data and saves it into a dictonary for caching
		results = api.user_timeline(user) 
		CACHE_DICTION[ident] = results #writes cached file 
		f = open(CACHE_FNAME,'w') 
		f.write(json.dumps(CACHE_DICTION)) #create a json dictionary into a file
		f.close() #closes file
	return CACHE_DICTION[ident]  #calls out specific data within the dictionary 



# Write an invocation to the functi on for the "umich" user timeline and save the result in a variable called umich_tweets:
...
db_cur.execute("CREATE TABLE Tweets (tweet_id TEXT PRIMARY KEY, tweet_text TEXT, user_id TEXT NOT NULL, time_posted TIMESTAMP, retweets INT)")
db_cur.execute("DROP TABLE IF EXISTS Users")
db_cur.execute("CREATE TABLE Users (user_id TEXT PRIMARY KEY, screen_name TEXT, num_favs INT, description TEXT)")
in_tweet = "INSERT INTO Tweets VALUES (?, ?, ?, ?, ?)"
in_use = "INSERT OR IGNORE INTO Users VALUES (?, ?, ?, ?)"
for tweets in umich_tweets:
	full = (tweets["id"], tweets["text"], tweets["user"]["id"], tweets["created_at"], tweets["retweet_count"])
	db_cur.execute(in_tweet, full)
	user = (tweets["user"]["id"], tweets["user"]["screen_name"], tweets["user"]["favourites_count"], tweets["user"]["description"])
	db_cur.execute(in_use, user)
	for m in tweets["entities"]["user_mentions"]:
		mentions = api.get_user(m["screen_name"])
		final_mention = (mentions["id"], mentions["screen_name"], mentions["favourites_count"], mentions["description"])
		db_cur.execute(in_use,final_mention)
...
db_cur.execute('SELECT screen_name FROM Users')
users_screen_names = db_cur.fetchall()
...
db_cur.execute('SELECT * FROM Tweets WHERE retweets > 5')
...
db_cur.execute('SELECT description FROM Users WHERE num_favs > 25')
descriptions_fav_users = [str(description[0]) for description in db_cur.fetchall()]
...
db_cur.execute("SELECT Users.screen_name, tweet_text FROM Users INNER JOIN Tweets ON Users.user_id=Tweets.user_id")
tups_twitter= db_cur.fetchall()
print(tups_twitter)
twitter_info_diction= {}
for t in tups_twitter:
